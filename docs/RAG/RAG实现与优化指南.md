...
用户上传文档 (PDF/MD/TXT)
       ...
       ▼
① 文件解析 → 提取纯文本
   - PDF: ParagraphPdfDocumentReader（优先，按段落提取）
         → 降级 PagePdfDocumentReader（按页提取）
   - MD/TXT: TextReader（纯文本直接读取）
       │
       ▼
② 文本清洗 (DocumentTextCleaner) ← 新增！
   - 规则化去噪：移除页眉页脚、日期戳、页码、重复标题行
...
④ 文本 → 向量化 (Embedding)
   ...
   - 语义相近的文本 → 向量距离近
       ...
       ▼
⑤ 向量 + 元数据 → PGVector 存储
   - metadata: {domain, title, file_type, doc_id, chunk_index}
       │
       ▼
...
| 清洗策略 | 说明 | 实现方式 |
|---------|------|---------|
| 页眉页脚移除 | 日期戳、页码、重复标题行 | 正则匹配 `^\\d{4}[/-]\\d{1,2}[/-]\\d{1,2}\\s+\\d{1,2}:\\d{2}.*$` |
| 空白符规范化 | 多空格→单空格，多空行→单空行 | `[ \\t]{2,}` → `" "`, `\\n{3,}` → `\\n\\n` |
| PDF 断行修复 | 中文行尾无标点时合并为连续段落 | 检测行尾中文字符 + 行首中文字符 → 合并 |
| 控制字符清理 | 移除不可见字符 | `[\\x00-\\x1F\\x7F-\\x9F]` → `""` |
| 短行过滤 | 移除过短无意义行 | 长度 < 3 且不含句末标点 → 移除 |

**为什么文本清洗是 RAG 最被低估的优化**：
- PDF 提取的原始文本包含 30%~50% 的格式噪声
- 噪声文本的 Embedding 语义模糊，直接拉低检索精度
- 清洗后向量质量提升 → 检索精度提升 → LLM 回答质量提升
- ROI 最高：代码量小，效果显著

### 3.2 智能分块优化（已实现）← 优化！

| 策略 | 旧方案 (TokenTextSplitter) | 新方案 (SmartTextSplitter) |
|------|---------------------------|---------------------------|
| 分块依据 | Token 数 | 段落优先 → 句子级别 |
| 重叠窗口 | 无 | 12.5%（业界推荐 10%~20%） |
| 中文感知 | 无 | 在句末标点处优先分割 |
| 超长文本 | 直接截断 | 递归切分到句子级别 |
| chunkSize | 800 Token | 800 字符（中文约 400~500 字） |

**重叠窗口 (Overlap) 的作用**：
- 问题：关键信息恰好在分块边界，一半在上一个块，一半在下一个块
- 解决：重叠区域确保边界附近的信息在两个块中都有完整上下文
- 代价：少量存储冗余（约 12.5%），但检索质量显著提升

### 3.3 PDF 解析优化（已实现）← 新增！

| 解析器 | 原理 | 适用场景 |
|--------|------|---------|
| ParagraphPdfDocumentReader | 利用 PDF 目录（TOC）按段落提取 | 有目录结构的 PDF（书籍、论文） |
| PagePdfDocumentReader | 按页提取（Apache PdfBox） | 无目录的 PDF（扫描件、简单文档） |

**降级策略**：优先尝试 ParagraphPdfDocumentReader，如果 PDF 无目录信息则降级为 PagePdfDocumentReader。

### 3.4 Chunking 参数调优（已实现）
